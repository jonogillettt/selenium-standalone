...
function computeDownloadUrls(opts, askedOpts) {
  ...
  if (opts.drivers.edge) {
    downloadUrls.edge = getEdgeDriverUrl(opts.drivers.edge);
  }
...
// legacy Edge releases, pre-Chromium
var microsoftEdgeReleases = require('./microsoft-edge-releases')

function getEdgeDriverUrl(opts) {
  // check legacy releases
  var release = microsoftEdgeReleases[opts.version]
  if (release) {
    return release.url
  }
