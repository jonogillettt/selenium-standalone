  ...
  function installEdgeDr(opts, cb) {
    if (path.extname(opts.from) === ".msi") {
      ...
      getDownloadStream(opts.from, function (err, stream) {
        ...
        const dlZipPath = opts.to + '.zip'
        // Store downloaded compressed file
        var zipWriteStream = fs
          .createWriteStream(dlZipPath)
          .once("error", cb.bind(null, new Error("Could not write to " + dlZipPath)));
        stream.pipe(zipWriteStream);

        // Uncompress downloaded file
        zipWriteStream.once("finish", function () {
          const zipStream = fs.createReadStream(dlZipPath).pipe(unzipper.Parse());
          zipStream.on('error', function (err) {
            console.error(err)
            cb(
              new Error("Could not unzip file " + dlZipPath + " to " + opts.to)
            );
          })
          ...
          function handleEntry(entry) {
            ...
            if (fileName.match(/\.exe$/i)) {
              entry.pipe(fs.createWriteStream(opts.to));
              zipStream.off("entry", handleEntry);
              cb();
            } else {
              entry.autodrain();
            }
          ...
          zipStream.on("entry", handleEntry);
        });
      });
    }
  }
